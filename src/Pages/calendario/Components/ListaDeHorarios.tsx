...
import { SHr, SImage, SLoad, SPage, SText, STheme, SView } from "servisofts-component";
    ...
    render() {
        return <FlatList
            ...
            ItemSeparatorComponent={() => {
                return <SView col={"xs-12"} height={2} style={{
                    borderBottomWidth: 1,
                    borderColor: STheme.color.lightGray
                }} />
            }}
            renderItem={({ index, item }) => {
                return <SView height={60} col={"xs-12"} row >
                    <SView width={100} height style={{
                        justifyContent: "center"
                    }}>
                        <SText font={"Montserrat-Medium"}>{item.label}</SText>
                    </SView>
            ...
            ListFooterComponent={() => {
                ...
                if (!this.state.data) return <SLoad />
                const dataDia = this.state.data[item.dia];
                return <EditarHorario key_restaurante={this.props.key_restaurante} dia={item} data={dataDia} onSave={e => {
